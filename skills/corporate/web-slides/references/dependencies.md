...
| 组件 | 锁定版本/来源 | 用途 | 许可 |
| --- | --- | --- | --- |
| PptxGenJS 浏览器完整包 | 4.0.1；npm 官方发布归档 | 原生文本、图片、形状、表格、图表及工作簿、讲稿、布局 | MIT |
...
| Babel polyfill、core-js、regenerator runtime | 完整包内嵌代码；core-js 标识为 2.6.11 | 上游保留的旧浏览器兼容代码 | MIT |

权威运行时文件为 `assets/vendor/pptxgenjs-4.0.1.bundle.js`，460,889 字节，SHA-256：

```text
4fb9eac5cfefb213e2d8743c2b7151025f31bfb3f834c73c12062916daa0f3f8
```

该文件来自 [PptxGenJS 4.0.1 发布归档](https://registry.npmjs.org/pptxgenjs/-/pptxgenjs-4.0.1.tgz) 的 `package/dist/pptxgen.bundle.js`，保持原始字节。上游完整包由 [固定版本构建脚本](https://github.com/gitbrent/PptxGenJS/blob/v4.0.1/gulpfile.js) 将 [libs](https://github.com/gitbrent/PptxGenJS/tree/v4.0.1/libs) 与导出器组合。并非运行时访问这些网址。

`assets/vendor/manifest.json` 记录分发文件校验和、归档来源及许可证参考包。内嵌的 Babel/regenerator 等代码未逐项提供版本标识，因此参考包版本用于取得对应版权许可文本，不冒充完整、可重建的 npm 锁文件；实际运行代码以整体归档与文件哈希锁定。所有对应许可证随包保留。分发完整 Skill 时保留 `assets/vendor/` 与此说明；单文件 HTML 的构建流程应包含许可证文本/来源说明。
