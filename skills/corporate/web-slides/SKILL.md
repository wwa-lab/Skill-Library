...
description: Use when creating corporate web presentations for internal sharing, training or roadshows, converting PPTX content, or reusing PPTX/POTX branding with offline editing and editable PowerPoint export. English is the default output language; follow an explicitly requested language.
license: Internal
metadata:
...
为内部分享、培训和路演制作英文优先、可继续编辑的离线网页演示，并导出原生可编辑 PPTX / POTX。用户明确指定其他语言时按其要求制作。沿用本地文件工作方式：不上传公司材料到转换服务，不自动安装工具，不发布文件。

## 输入和默认值

接收主题、文档、Markdown、已有 PPTX、公司 PPTX/POTX 模板，或本 Skill 生成的 HTML。旧 `.ppt` 请先另存为 `.pptx`。普通第三方 HTML 不执行其中的脚本；先由 Agent 读取并转成内容模型。

没有重大歧义时直接继续：新建演示默认英文、现场分享、16:9、红白黑品牌、每页一个主题、完整英文讲稿。生成的工具栏、编辑面板、演讲者界面和运行时提示也默认英文。用户明确指定其他目标语言时按其要求；把已有演示转换为网页时保留源语言。其他材料用于新建演示而目标语言未指定时，忠实整理为英文，保留专名、数字和事实口径。根据内容决定页数；用户给定页数和风格优先。可选科技主题为 `tech-cyan`（深蓝青色）、`digital-violet`（深紫）、`tech-light`（浅色蓝白），适配已有品牌并保留 Logo；这些是演示风格，不表示品牌官方批准。品牌默认为 `assets/default-brand.json`，不虚构公司 Logo、业绩、来源或事实。主题/文档的内容提炼使用用户批准的 Agent/模型；演示、编辑、保存和导出无需模型或网络。
...
7. **验收和交付。** 按 [验收清单](references/quality-checks.md)检查离线打开、文字溢出、页面顺序、讲稿、保存重开、原生对象和母版。执行可用的必要测试，把环境未覆盖项标为“未验证”。交付 HTML、PPTX，复用场景加 POTX、品牌 JSON，以及转换报告/验证记录。无需为不可用的 Office 或 Windows 安装、升级软件。
...
- 字体不随包嵌入；默认英文主题使用 Arial，并提供系统 sans-serif 回退。明确选择的品牌字体优先。中英混排须检查字形回退；网页/PPTX 的字体度量、图表细节可能不同，须分别检查。
