...
3. **建立统一模型。** 读 [模型约定](references/model-contract.md)，从 `assets/deck-template.json` 或 `examples/six-slides.json` 复制。文字用纯文本、图片用本地 PNG/JPEG 或 data URL，表格和基础图表使用结构化数据。只编辑模型；DOM 和 PPTX 都是派生结果。不要将整页截图伪装成可编辑 PPTX。
...
6. **浏览器编辑与导出。** 打开 HTML，展示/编辑模式可切换。支持标题、正文、讲稿、独立图片替换和裁切位置、字号/颜色/对齐、页面增删复制排序、撤销重做；表格/图表数据可在对象面板更新。保存下载完整 HTML 后重开继续编辑。PPTX/POTX 导出必须读取最新模型；缓存仅作恢复辅助。细则见 [使用说明](README.md)。
...
- 1600×900 设计坐标，整页等比缩放；桌面和投影采用留边，不重排页面。文字过密优先减字或拆页，不能仅用隐藏溢出掩盖。
...
`examples/six-slides.html`、`six-slides.pptx`、`six-slides.potx` 和模型覆盖六页功能；测试记录见 [验收记录](references/test-record.md)。它们证明已记录环境中的行为，不替代公司 Windows、Edge/Chrome 策略及实际 PowerPoint 验收。
